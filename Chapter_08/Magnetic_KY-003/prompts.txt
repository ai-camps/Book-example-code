...
•	Reads the digital value (HIGH or LOW) from the hall magnetic sensor every 1 second.
•	Beep the Piezo Buzzer and change LED color to indicate if magnetic is detected.

Requrements:
•	Use PlatformIO as your IDE.
•	Connect ESP32 GPIO pin 0 to the D0 pin of KY-003 sensor.
...
•	If the value is HIGH, that indicates magnetic field is detected, beep the Piezo Buzzer, and change the LED color to red.
•	If the value is LOW, that indicates no magnetic field is detected, mute the Piezo Buzzer, and maintain the LED color to green.
