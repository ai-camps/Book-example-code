...
•	Reads the digital value (HIGH or LOW) from the switch sensor every 1 second.
•	Beep the Piezo Buzzer and change LED color to indicate if collision is detected.

Requirements:
•	Use PlatformIO as your IDE.
•	Connect ESP32 GPIO pin 0 to the D0 pin of switch sensor.
...
•	If the value is LOW, that indicates collision is detected, beep the Piezo Buzzer, and change the LED color to red.
•	If the value is HIGH, that indicates no collision is detected, mute the Piezo Buzzer, and maintain the LED color to green.
