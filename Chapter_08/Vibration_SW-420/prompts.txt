...
•	Reads the digital value (HIGH or LOW) from SW-420 every 1 second.
•	Beep the Piezo Buzzer and change LED color to indicate if vibration is detected.


Requirements:
•	Use PlatformIO as your IDE.
•	Connect ESP32 GPIO pin 0 to the D0 pin of SW-420.
...
•	If the value is HIGH, that indicates vibration is detected, beep the Piezo Buzzer, and change the LED color to red.
•	If the value is LOW, that indicates no vibration is detected, mute the Piezo Buzzer, and maintain the LED color to green.
